...
public class CoordinateTransformUtils {
    ...
    public static Coordinate gcj02ToWgs84Exactly(double lng, double lat) {
        ...
        while (true) {
            wgsLat = (mLat + pLat) / 2;
            wgsLng = (mLon + pLon) / 2;
            Coordinate coordinate = wgs84ToGcj02(wgsLng, wgsLat);
            dLon = coordinate.getLng() - lng;
            dLat = coordinate.getLat() - lat;
    ...
    public static Coordinate wgs84ToBd09(double lng, double lat) {
        Coordinate coordinate = wgs84ToGcj02(lng, lat);
        return gcj02ToBd09(coordinate.getLng(), coordinate.getLat());
    }

    // BD09=>WGS84 百度坐标系=>地球坐标系
    public static Coordinate bd09ToWgs84(double lng, double lat) {
        Coordinate coordinate = bd09ToGcj02(lng, lat);
        return gcj02ToWgs84(coordinate.getLng(), coordinate.getLat());
    }
